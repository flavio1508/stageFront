...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://stagebackend-1.onrender.com/api/projects/${projectId}`, { withCredentials: true })
    .then(response => {
    ...
    axios.get(`https://stagebackend-1.onrender.com/api/members/${projectId}`, { withCredentials: true })
    .then(response => {
    ...
    axios.get(`https://stagebackend-1.onrender.com/api/files/folders/${projectId}`)
    .then(response => {
      ...
      foldersWithFiles.forEach((folder, index) => {
        axios.get(`https://stagebackend-1.onrender.com/api/files/${projectId}/${folder.id}`)
          .then(fileResponse => {
    ...
    axios.get(`https://stagebackend-1.onrender.com/api/projectMaterials/${projectId}`, { withCredentials: true })
      .then(response => {
    ...
    axios.get('https://stagebackend-1.onrender.com/api/users', { withCredentials: true })
      .then(response => {
  ...
  const handleAddMember = (memberEmail) => {
    ...
    if (selectedUser) {
      ...
      axios.post(`https://stagebackend-1.onrender.com/api/members/${projectId}`, { member: selectedUser.id })
        .then(response => {
  ...
  const handleRemoveMember = (index) => {
    ...
    axios.delete(`https://stagebackend-1.onrender.com/api/members/${projectId}/${memberToRemove.id}`)
      .then(() => {
  ...
  const handleAddBudgetItem = () => {
    if (editIndex >= 0) {
      ...
      axios.put(`https://stagebackend-1.onrender.com/api/projectMaterials/${projectMaterials[editIndex].id}`, updatedItem)
        .then(response => {
      ...
      axios.post(`https://stagebackend-1.onrender.com/api/projectMaterials`, newItem)
        .then(response => {
  ...
  const handleRemoveBudgetItem = (index) => {
    ...
    axios.delete(`https://stagebackend-1.onrender.com/api/projectMaterials/${itemToRemove.id}`)
      .then(() => {
  ...
  const handleAddFolder = () => {
    ...
    if (folderName) {
      axios.post(`https://stagebackend-1.onrender.com/api/files/folders/${projectId}`, { name: folderName })
        .then(response => {
  ...
  const handleRemoveFolder = (folderIndex) => {
    ...
    axios.delete(`https://stagebackend-1.onrender.com/api/files/folders/${projectId}/${folderToRemove.id}`)
      .then(() => {
  ...
  const handleAddFileToFolder = (folderIndex, file) => {
    ...
    axios.post(`https://stagebackend-1.onrender.com/api/files/${projectId}/${folders[folderIndex].id}`, formData)
      .then(response => {
  ...
  const handleRemoveFile = (folderIndex, fileIndex) => {
    ...
    axios.delete(`https://stagebackend-1.onrender.com/api/files/${projectId}/${folders[folderIndex].id}/${fileToRemove.id}`)
      .then(() => {
        const updatedFolders = [...folders];
        updatedFolders[folderIndex].files.splice(fileIndex, 1);
        setFolders(updatedFolders);
                ...
                <ul className="files-list">
                  {Array.isArray(folder.files) && folder.files.map((file, fileIndex) => (
                    <li key={fileIndex} draggable onDragStart={(e) => handleDragStart(e, folderIndex, fileIndex)} className="file-item">
                      <FaFile className="file-icon"/><a className="filesPastas" href={`https://stagebackend-1.onrender.com/api/files/download/${file.id}`} download={file.name} >
                        {file.name}
                      </a>
